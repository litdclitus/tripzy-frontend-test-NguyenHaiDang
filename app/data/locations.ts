export interface Location {
  short_code: string;
  english_name: string;
  code_state: string;
}

export const locations: Location[] = [
  {
    short_code: "MD",
    english_name: "Moldova, Republic of",
    code_state: "+373 - Moldova, Republic of",
  },
  {
    short_code: "MC",
    english_name: "Monaco",
    code_state: "+377 - Monaco",
  },
  {
    short_code: "AN",
    english_name: "Netherlands Antilles",
    code_state: "+599 - Netherlands Antilles",
  },
  {
    short_code: "NC",
    english_name: "New Caledonia",
    code_state: "+687 - New Caledonia",
  },
  {
    short_code: "NZ",
    english_name: "New Zealand",
    code_state: "+64 - New Zealand",
  },
  {
    short_code: "NU",
    english_name: "Niue",
    code_state: "+683 - Niue",
  },
  {
    short_code: "NF",
    english_name: "Norfolk Island",
    code_state: "+672 - Norfolk Island",
  },
  {
    short_code: "MP",
    english_name: "Northern Mariana Islands",
    code_state: "+1 670 - Northern Mariana Islands",
  },
];

// Helper function to filter locations based on search query
export const filterLocations = (
  searchValue: string,
  allLocations: Location[]
): Location[] => {
  if (!searchValue) return allLocations;

  const searchLower = searchValue.toLowerCase();
  return allLocations.filter(
    (location) =>
      location.short_code.toLowerCase().includes(searchLower) ||
      location.english_name.toLowerCase().includes(searchLower) ||
      location.code_state.toLowerCase().includes(searchLower)
  );
};

// Helper function to get locations excluding a specific one
export const getAvailableLocations = (
  excludeLocation: string,
  allLocations: Location[]
): Location[] => {
  if (!excludeLocation) return allLocations;

  return allLocations.filter(
    (location) =>
      location.english_name !== excludeLocation &&
      location.short_code !== excludeLocation &&
      location.code_state !== excludeLocation
  );
};

